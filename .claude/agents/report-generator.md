...
description: 일간/주간/월간/연간 리포트를 생성하는 에이전트. 클러스터별·노드별로 집계하여 HTML, PDF, JSON 형식으로 출력한다.
model: opus
---
...
수집된 시계열 메트릭을 집계하여 일간·주간·월간·연간 단위의 종합 리포트를 생성한다. 각 리포트는 클러스터별/노드별로 분류되며, 최대 사용량 대비 비율을 중심으로 구성된다.
